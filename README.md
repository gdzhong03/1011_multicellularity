...
2. `CCM_Model_Train.ipynb`: Trains and evaluates a seven-class MobileNetV2 image classifier for CCM phenotypes, generates a confusion matrix, and exports the trained model.
...
4. `PSH_Categorization_Model_Train.ipynb`: Trains and evaluates a four-class MobileNetV2 model that categorizes well images as no growth, no PSH, PSH, or contamination, then exports the model.
5. `PSH_Categorization_Model_Predict.ipynb`: Uses the trained seven-class CCM model to classify well images, sorts copies into predicted-class folders, and saves class and confidence results to Excel.
